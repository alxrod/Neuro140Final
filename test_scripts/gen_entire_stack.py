...
model.load_state_dict(torch.load("seg_checkpoints/model_epoch_40.pth"))
...
for images, masks in tqdm(dataloader_train):
  ...
  for i in range(len(masks)):
    ...
    imsave("final_predictions/final_pred_"+str(epoch)+"_"+raw_name+".png",np.squeeze(pred_binary_mask))
    imsave("final_predictions/final_gt_"+str(epoch)+"_b"+raw_name+".png",np.squeeze(gt_binary_mask))
